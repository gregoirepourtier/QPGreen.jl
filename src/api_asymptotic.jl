...
# !!! Note that through this asymptotic API, only the implementation of the eval and grad functions are provided. The hess functions are not implemented yet. Moreover these functions are not exported.
...
# Returns
    ...
    - If `grad=true`: a NamedTuple with fields
            ...
            + `cache`: Precomputed integration cache for reuse in later computations.
"""
function init_qp_green_fft_asymptotic(params::NamedTuple, grid_size::Integer; grad=false)
    ...
    L̂ⱼ₂ = grad ? similar(L̂ⱼ) : nothing
    Φ₃_eval = grad ? similar(Φ₁_eval) : nothing
    h₁_reduced_eval = grad ? similar(Φ₁_eval) : nothing
    h₂_reduced_eval = grad ? similar(L̂ⱼ) : nothing
    ρₓ_eval = grad ? similar(Φ₁_eval) : nothing
    ...
    Φ̂₃_freq = grad ? similar(Φ̂₁_freq) : nothing

    ĥ₁ⱼ = grad ? Matrix{Complex{T}}(undef, N ÷ 2 + 1, N) : nothing
    ĥ₂ⱼ = grad ? Matrix{Complex{T}}(undef, N, N) : nothing

    ρ̂ₓⱼ = grad ? Matrix{Complex{T}}(undef, N ÷ 2 + 1, N) : nothing
    ...
    Ĥ₁ⱼ₀::Union{Nothing, Complex{T}} = grad ?
...
function grad_smooth_qp_green_asymptotic(x, params::NamedTuple, grad::NamedTuple{T1, T2}, Yε_cache::IntegrationCache;
    ...
    if abs(x[2]) > c
        ...
        return eigfunc_expansion_grad(x, params; nb_terms=nb_terms) .+ singularity .* x
    else
        t = get_t(x[1])
